...
st.html(
    ...
    /* Hide the +/- stepper buttons on number inputs across all browsers */
...
# ---------------- FREEZER ANALYSIS ----------------
    ...
    if daily_df_fa.empty:[cite: 1]
        ...
        oc1.metric("Total freezer stock", f"{int(round(total_stock))} units")[cite: 1, 2]
        oc2.metric("Avg. daily sales (all flavours)", f"{int(round(total_rate))} units/day")[cite: 1, 2]
        oc3.metric("Overall days of stock left", f"{int(round(overall_days_left))}" if overall_days_left is not None else "—")[cite: 1, 2]
...
# ---------------- DASHBOARD ----------------
    ...
    if not daily_df.empty:[cite: 2]
        ...
        st.metric("Stock across carts", f"{int(round(daily_df.sort_values('Date').groupby('Cart').tail(1)['Closing_Total'].sum()))}")[cite: 2]
